...
# Use MySQL if DB_URI is set, else fallback to SQLite
...
def chat():
    ...
    resources = []
    if sentiment == 'negative' or intent in ['sad', 'anxious', 'stress', 'crisis_suicide']:
        relevant_resources = Resource.query.filter(
            (Resource.sentiment_tag == 'negative') | 
            (Resource.category.in_(['anxiety', 'depression', 'motivation', 'safety']))
        ).limit(3).all()
